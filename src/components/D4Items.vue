...
const updateItem = ({ name, quantity, tier, quality, itemType, itemTypeValue1, itemTypeValue2, imageId, power, upgrade, level, price }: IItem): void => {
  ...
  if (changeTypeNotFirst) {
    ...
    activatedItem.value.imageId = Math.floor(Math.random() * itemImgs[itemType]?.[itemTypeValue1 as string]) || 0
...
// about screen size
const size = computed(() => $q.screen.width < 300 ? 'width:200px;height:200px;' : $q.screen.width < 336 ? 'width:300px;height:250px;' : $q.screen.width < 728 ? 'width:336px;height:280px;' : 'width:728px;height:90px;')
    ...
    <div :class="$q.screen.lt.sm ? 'q-gutter-y-xl' : 'q-gutter-y-xxl'">
      ...
      <q-intersection v-for="item, idx in (items as Array<Item>)" :key="item.itemId" :data-itemid="item.itemId"
        ...
        <div v-if="item.itemId === 'advertise' && $q.screen.lt.lg" class="row justify-center">
          <Adsense :style="size" data-ad-client="ca-pub-5110777286519562" data-ad-slot="6163086381" :data-adtest="!prod"
