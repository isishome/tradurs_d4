  ...
  <div class="col-12" :style="`max-width:${width}px`">
    ...
    `${itemHeight}px`}`" data-itemid="reward-item">
        <D4Item :data="rewardItem" :loading="rewardItem.loading" @favorite="favorite" @copy="copy"
          ...
          <template
            v-if="rewardItem.itemTypeValue1 === 'summoning' && descriptions(rewardItem.itemTypeValue2).length > 0"
            #description>
            <D4Material v-for="material in descriptions(rewardItem.itemTypeValue2)" :key="material.value"
              :data="material" />
          </template>
