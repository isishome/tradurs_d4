...
const optionStyle = computed(() =>
  !$q.platform.is.mobile
    ? { maxWidth: `${presetRef.value?.$el.offsetWidth}px` }
    : {}
)
...
<template>
      ...
      >
        <q-card :dark="!$q.dark.isActive">
          <q-card-section>
            <q-form
              autofocus
              class="row items-center justify-between q-gutter-x-sm"
              @submit="add"
            >
              <q-input
                :dark="false"
  ...
  <q-item :disable="disable">
            ...
            <q-item clickable :style="optionStyle" v-bind="scope.itemProps">
              <q-item-section side>
                <q-btn
                  ...
                  @click.stop="remove(scope.opt.value)"
                >
                  <img
                    class="negative"
                    width="13"
    ...
    <q-item-section side>
      <q-btn
        size="sm"
        :disable="disable || options.length === 5"
        unelevated
        dense
        round
...
<style scoped>
.negative {
  filter: invert(11%) sepia(78%) saturate(7404%) hue-rotate(349deg)
