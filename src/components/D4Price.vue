...
<template>
  <div v-if="editable">
    <div class="row items-center q-gutter-x-sm">
      ...
      <q-select v-model="_price.currency" class="col" outlined dense no-error-icon hide-bottom-space emit-value
        map-options transition-show="none" transition-hide="none" label="화폐 유형" :dropdown-icon="`img:${icons.dropdown}`"
        :options="currencies" @update:model-value="updateCurrency">
        <template #selected-item="scope">
          <div class="ellipsis">{{ scope.opt.label }}</div>
        </template>
      </q-select>
      <q-select v-if="data.currency === 'rune'" v-model="_price.currencyValue" class="col" outlined dense no-error-icon
        hide-bottom-space emit-value map-options transition-show="none" transition-hide="none" :options="runes()"
      ...
      <D4Counter v-if="data.currency !== 'offer'" v-model="_price.quantity" class="col" @update:model-value="update" />
    </div>
  </div>
  <template v-else-if="offer">
    ...
    <q-item v-show="!loading" class="q-my-lg">
      <q-item-section avatar>

        <q-avatar class="text-uppercase">
          <q-img src="https://cdn.quasar.dev/img/boy-avatar.png" />
        </q-avatar>
      </q-item-section>
      <q-item-section>
        <q-item-label>{{ data.userId }}</q-item-label>
        <q-item-label caption lines="1">
          ...
          <div v-else class="row items-center q-gutter-x-xs">
            ...
            <template v-else>
              ...
              <div class="q-ml-xs">{{(runes().find(r => r.value === data.currencyValue) || {}).label}}
              </div>
            </template>
            <div>x</div>
      ...
      <q-item-section side>
        <D4Btn label="수락" color="var(--q-secondary)" disable />
      </q-item-section>
    </q-item>
  </template>
  <div v-else>
