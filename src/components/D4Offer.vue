...
const acceptOffer = () => {
  $q.dialog({
    ...
    message: `<div class="text-subtitle1">${t(
      "accept.msg1"
    )}</div><div class="q-mt-xs text-caption">${t(
      "accept.msg2"
    )}<strong class="text-negative">${t("accept.msg3")}</strong>${t(
      "accept.msg4"
    )}<strong class="text-negative">${t("accept.msg5")}</strong>${t(
      "accept.msg6"
    )}</div>`,
    html: true,
    persistent: true,
...
const retractOffer = () => {
  $q.dialog({
    ...
    message: `<div class="text-subtitle1">${t(
      "retract.msg"
    )}</div><div class="q-mt-md text-caption text-negative text-right">${t(
      "retract.desc"
    )}</div>`,
    html: true,
    persistent: true,
...
const isAcceptable = computed(
  () =>
    props.data.itemStatusCode === "000" &&
    props.data.statusCode === "000" &&
    props.owner
)
const isRetractable = computed(
  () =>
    ...
    props.data.retractable
)
const isTradeable = computed(
  () =>
    (props.data.statusCode === "003" &&
      props.data.itemStatusCode === "003" &&
      props.data.authorized) ||
    (["001", "003"].includes(props.data.statusCode) &&
      props.data.itemStatusCode === "003" &&
      props.data.evaluations.length === 0 &&
      props.owner)
)
const existsEvaluation = computed(
  () =>
    (props.data.statusCode === "001" && props.owner) ||
    (props.data.statusCode === "001" && props.data.authorized)
)
const status = computed(() => is.findOfferStatus(props.data.statusCode)?.label)
const parsEvaluations = computed(() =>
  props.owner
    ? as.filterEvaluations(props.evaluations)
    : props.data.authorized
    ? as.filterEvaluations(props.data.evaluations)
    : []
)
</script>
    ...
    <q-item class="q-px-sm">
      <q-item-section v-show="data.loading">
        <q-skeleton
          class="self-end"
          width="100px"
          :height="$q.screen.lt.sm ? '24px' : '36px'"
        />
      </q-item-section>
      <q-item-section v-show="!data.loading">
        <div class="row justify-end items-center q-gutter-x-sm">
          ...
          <div v-else class="row items-start q-col-gutter-xs">
            <div>{{ status }}</div>
            <q-icon
              v-if="existsEvaluation"
              class="icon"
              name="img:/images/icons/evaluation.svg"
              size="19px"
            >
              <D4Tooltip>
                <div class="text-overline text-weight-bold">
                  {{
                    parsEvaluations.length === 0
                      ? t("offer.noManners")
                      : t("complete.evaluate")
                  }}
                </div>
                <div
                  v-show="parsEvaluations.length > 0"
                  class="break-keep text-caption"
                >
                  <ul class="evaluation">
                    <li
                      v-for="(evaluation, idx) of parsEvaluations"
                      :key="idx"
