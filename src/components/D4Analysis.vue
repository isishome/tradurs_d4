...
const checkAffixes = (textStr: string) => {
  ...
  for (const affix of is.affixes.data) {
    const matchAffix = textStr.match(new RegExp(affix.label.replace(/(\[[0-9.]{1,}[\s\-]{1,}[0-9.]{1,}\])|[\+ ]/g, '').replace(/\{x\}/g, '[0-9.]{1,}') as string, 'i'))
...
const checkRestrictions = () => {
  ...
  for (const restriction of is.restrictions.data) {
    const matchRestriction = restrictionsPhase.match(new RegExp(restriction.label.replace(/(\[[0-9.]{1,}[\s\-]{1,}[0-9.]{1,}\])|[\+ ]/g, '').replace(/\{x\}/g, '[0-9.]{1,}') as string, 'i'))
