...
<template>
    <div v-if="editable">
        <div class="row items-center q-gutter-x-sm">
            ...
            <q-select v-model="_price.currency" class="col" outlined dense no-error-icon hide-bottom-space emit-value
                map-options transition-show="none" transition-hide="none" label="화폐 유형"
                :dropdown-icon="`img:${icons.dropdown}`" :options="currencies" @update:model-value="updateCurrency">
                <template #selected-item="scope">
                    <div class="ellipsis">{{ scope.opt.label }}</div>
                </template>
            </q-select>
            <q-select v-if="data.currency === 'rune'" v-model="_price.currencyValue" class="col" outlined dense
            ...
            <Counter v-if="data.currency !== 'offer'" v-model="_price.quantity" class="col"
    ...
    <template v-else-if="offer">
        ...
        <q-item v-show="!loading" class="q-my-lg">
            ...
            <q-item-section side>
                <q-btn dense unelevated color="red-10" :disable="loading" class="no-hover">
                    <img class="invert" width="24" height="16" src="@/assets/icons/more.svg" />
                </q-btn>
            </q-item-section>
        </q-item>
    </template>
