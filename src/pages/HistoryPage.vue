...
const periodOptions = Array.from({ length: 3 }, (_, i) => ({
  value: i,
  label: date.formatDate(date.subtractFromDate(today, { months: i }), "YYYY-MM")
}))
...
<template>
  <div>
    ...
    <q-list
      ...
      class="contents"
      :class="
        $q.screen.lt.sm ? 'q-py-md q-pl-sm q-pr-lg' : 'q-py-lg q-pl-md q-pr-xl'
      "
    >
      <q-timeline>
        <q-timeline-entry
          class="q-pl-sm"
          :tag="$q.screen.lt.sm ? 'h6' : 'h5'"
          heading
        >
          <q-skeleton v-show="loading" width="100px" height="24px" />
          <div v-show="!loading">
            {{ periodOptions.find((po) => po.value === period)?.label }}
          </div>
        </q-timeline-entry>
          ...
          <template v-slot:title>
            <div class="row items-center q-gutter-x-md text-body1">
              ...
              <div v-if="historyType === 'temperature'">
                {{
                  (h.contents.degree as number) > 0
                    ? t("history.temperatureRise", {
                        degree: h.contents.degree
                      })
                    : t("history.temperatureDrop", {
                        degree: h.contents.degree
                      })
                }}
              </div>
              <div v-else-if="historyType === 'yolk'">
                {{
                  (h.contents.amount as number) > 0
                    ? t("history.yolkRise", { amount: h.contents.amount })
      ...
      <div v-show="more" class="row justify-center items-center">
        <q-btn
          ...
          no-caps
          unelevated
          :aria-label="t('btn.showMore')"
          :ripple="false"
          class="no-hover"
          :label="t('btn.showMore')"
          color="primary"
          @click="request()"
        />
      </div>
...
.tab:deep(button) {
  ...
  box-shadow: inset 0 1px 0 0 rgba(255, 255, 255, 0.28),
    inset 1px 0 0 0 rgba(255, 255, 255, 0.28),
    inset -1px 0 0 0 rgba(255, 255, 255, 0.28);
...
.body--light .tab:deep(button) {
  box-shadow: inset 0 1px 0 0 rgba(0, 0, 0, 0.12),
    inset 1px 0 0 0 rgba(0, 0, 0, 0.12), inset -1px 0 0 0 rgba(0, 0, 0, 0.12);
...
.tab:deep(.bg-active) {
  box-shadow: inset 0 1px 0 0 rgba(255, 255, 255, 0.28),
    inset 1px 0 0 0 rgba(255, 255, 255, 0.28),
    inset -1px 0 0 0 rgba(255, 255, 255, 0.28), 0 1px 0 0 var(--q-dark-page);
