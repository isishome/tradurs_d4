...
const copy = (itemId: string) => {
  ...
  if (findItem) itemsRef.value?.copyItem(findItem)
}
...
const selectAll = (val: boolean) => {
  ...
  if (rewardItem.value) rewardItem.value.selected = val
}
        ...
        if (erroritems.length > 0) {
          ...
          errorInfo.items.push(
            ...erroritems.map((i) => ({
              ...i,
              ...itemInfo(
                items.value.find((it) => it.itemId === i.itemId.toString())
              )
            }))
          )
        }
...
const statusItems = (status: string) => {
  ...
  $q.dialog({
    ...
    selectAction.value = false
    is.statusItems(
      items.value.filter((i) => i.selected).map((i) => i.itemId),
      status
    )
      .then((erroritems: Array<IErrorItem>) => {
        if (erroritems.length > 0) {
...
watch(filter, (val, old) => {
  if (Number.isInteger(val) && val !== old) reload()
})
...
<template>
  <div ref="listHeaderRef">
    <div
      ...
      :style="`transform: translateY(-${gs.offsetTop}px);`"
    ></div>
    <div
      class="row justify-between items-start absolute full-width q-px-md list-header"
      :class="{ scroll: butted }"
      :style="`top:${gs.offsetTop}px`"
    >
      <div class="col row items-center q-gutter-x-sm">
        <q-btn-dropdown
          v-if="as.signed && is.filter.mine"
        ...
        >
          ...
          <q-list
            ...
            class="text-caption no-border"
            :class="[
              { 'cursor-not-allowed': noSelected },
              $q.dark.isActive
          ...
          >
            <q-item
              :dark="false"
              :class="{ 'no-pointer-events disabled': noSelected }"
              clickable
              @click="relistItems"
            >
      ...
      <div class="col row justify-end items-center overflow-hidden">
        <q-select
          ...
          menu-self="top end"
          dropdown-icon="img:/images/icons/dropdown.svg"
          class="sort text-caption"
          popup-content-class="scroll bordered text-caption"
              ...
              <li v-for="item in errorInfo.items" :key="item.itemId">
                <div class="column items-start">
                  <div
                    ...
                    :style="`color:var(--q-light-${item.quality})`"
                  >
                    {{ item.name }}
                  </div>
...
.list-header.scroll {
  ...
  box-shadow: 0 -1px 1px 0 var(--q-dark), 0 1px 1px 0 rgb(100, 100, 100),
    1px 0 1px 0 var(--q-dark-border), -1px 0 1px 0 var(--q-dark-border);
}

.body--light .list-header.scroll {
  background-color: var(--q-light);
  box-shadow: 0 -1px 1px 0 var(--q-light), 0 4px 4px 2px rgba(0, 0, 0, 0.1),
...
.item-list:deep(
    .item:not(.reward) .card-item:not(.editable) .filtered .minmax-text
  ) {
