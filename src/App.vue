...
export default {
  preFetch({ store }) {
    ...
    return Promise.all([as.checkSign(), as.getHistoryTypes(), as.getEvaluations(), is.getStorage(), is.getBase(), is.getProperties(), is.getAffixes(), is.getRestrictions(), is.getPacts()]);
  }
}
</script>
...
const notice = reactive<{ open: boolean, close: boolean }>({
  open: false,//!$q.cookies.has('d4.update.20231020'),
  close: false
})

const close = () => {
  $q.cookies.set('d4.update.20231020', 'confirm', { expires: 1, path: '/' })
  notice.open = false
}
